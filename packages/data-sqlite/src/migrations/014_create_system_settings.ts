...
export function up(db: any): void {
  ...
  try {
    db.exec(`ALTER TABLE users ADD COLUMN asr_config TEXT DEFAULT '{}';`);
  } catch {}
  ...
  db.exec(`
    ...
    -- 插入ASR提供商定义
    INSERT OR IGNORE INTO asr_providers (id, name, display_name, provider_type, description, default_config, required_fields, is_enabled, created_at, updated_at) VALUES
      ('deepgram', 'deepgram', 'Deepgram', 'deepgram', 'Deepgram实时语音识别服务', 
        '{"model":"nova-2","language":"zh","punctuate":true,"profanity_filter":false,"redact":false,"diarize":false,"numerals":true,"endpointing":400,"interim_results":true,"utterance_end_ms":1000}',
        '["apiKey"]',
        true, ${Date.now()}, ${Date.now()}),
      ('openai_whisper', 'openai_whisper', 'OpenAI Whisper', 'openai_whisper', 'OpenAI Whisper语音识别服务',
        '{"model":"whisper-1","language":"zh","temperature":0,"response_format":"json"}',
        '["apiKey"]',
        true, ${Date.now()}, ${Date.now()}),
      ('vosk', 'vosk', 'Vosk', 'vosk', 'Vosk本地语音识别服务',
