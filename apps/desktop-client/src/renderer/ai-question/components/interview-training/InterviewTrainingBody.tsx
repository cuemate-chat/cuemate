...
export function InterviewTrainingBody({ aiMessage, candidateAnswer, isLoading, message, isError, status }: WindowBodyProps) {
        ...
        {!displayAiMessage && !isLoading ? (
          ...
          <div className="ai-empty-state">
            ...
            <div className="ai-empty-title">
              {status === 'interview-training-paused' ? '训练已暂停'
                : status === 'interview-training-completed' ? '训练已结束'
                : status === 'interview-training-error' ? '训练出错'
                : status === 'interview-training-expired' ? '训练已过期'
                : '等待训练开始'}
            </div>
