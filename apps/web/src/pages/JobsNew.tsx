...
import { Button, Input, Modal, Steps, Tabs, Upload } from 'antd';
...
export default function JobsNew() {
  ...
  const handleCopySample = async () => {
    try {
      ...
      globalMessage.error('复制失败，请手动选择文本复制');
    }
  };
            ...
            <div className="mt-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  <Button size="small" onClick={handleApplySampleToCurrent}>一键带入当前简历</Button>
                  {prevResumeText !== null && (
                    <Button size="small" onClick={handleUndoApplySample}>回退之前内容</Button>
                  )}
                  <Button size="small" type="primary" onClick={handleCopySample}>复制</Button>
                </div>
              </div>
              <TextArea value={resumeSample} rows={12} readOnly />
            </div>
          </div>
        </Modal>
      </div>
