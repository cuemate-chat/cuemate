...
const log = createModuleLogger('RegoloProvider');
const PROVIDER_NAME = 'Regolo';

export class RegoloProvider extends BaseLLMProvider {
  constructor() {
    super('regolo');
  }
  ...
  async complete(request: CompletionRequest, config: RuntimeConfig): Promise<CompletionResponse> {
    ...
    const baseUrl = config.credentials.base_url || 'https://api.regolo.ai/v1';
    ...
    try {
      ...
      log.error('complete', 'Regolo completion failed', {}, error);
      throw error;
    }
  }
  ...
  async *stream(request: CompletionRequest, config: RuntimeConfig): AsyncGenerator<string> {
    ...
    try {
      ...
      log.error('stream', 'Regolo stream failed', {}, error);
      throw error;
    }
  }
  ...
  async healthCheck(config: RuntimeConfig): Promise<boolean> {
    ...
    try {
      ...
      log.error('healthCheck', `Regolo health check failed for model ${config.model}`, {}, error);
      throw error;
    }
  }
}
