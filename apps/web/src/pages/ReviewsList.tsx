                    ...
                    <div className="flex items-start justify-between gap-3 pl-4">
                      <div className="min-w-0">
                        ...
                        <div className="mt-3 flex flex-wrap gap-3 text-sm">
                          {(() => {
                            const toFixed15 = (val?: string) => {
                              const text = (val || '—').slice(0, 15);
                              const padLen = Math.max(0, 15 - text.length);
                              return text + '　'.repeat(padLen);
                            };
                            return (
                              <>
                                <Tag color="blue" className="!m-0 !px-3 !py-1 font-medium whitespace-pre">{toFixed15(it.advantage_content || it.overall_suggestions)}</Tag>
                                <Tag color="red" className="!m-0 !px-3 !py-1 font-medium whitespace-pre">{toFixed15(it.disadvantage_content || it.overall_cons)}</Tag>
                              </>
                            );
                          })()}
                        </div>
