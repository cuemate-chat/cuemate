...
export class EmbeddingService {
  constructor(_config: Config['embeddings']) {
    // 不需要任何配置，因为我们使用 ChromaDB 的默认嵌入函数
  }
  ...
  async embed(texts: string[]): Promise<number[][]> {
    ...
    return texts.map((text) => {
      ...
      // 生成一个 384 维的向量（ChromaDB 的默认维度）
      const vector = new Array(384).fill(0);

      // 对每个词，使用其字符的 Unicode 码点影响向量的多个维度
      words.forEach((word) => {
        if (!word) return;
        ...
        for (let i = 0; i < 8; i++) {
          ...
          const value = ((hash >> (i * 4)) & 0xf) / 0xf; // 0-1 之间的值
          vector[pos] = (vector[pos] + value) / 2;
        }

        // 对每个字符，使用其 Unicode 码点影响向量的某些维度
        for (let i = 0; i < word.length; i++) {
          const code = word.charCodeAt(i);
          const pos = code % 384;
          vector[pos] = (vector[pos] + code / 65535) / 2; // 65535 是 Unicode 的最大值
        }
      });
