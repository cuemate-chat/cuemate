...
export default function CollapsibleSidebar({
  ...
  return (
    ...
    >
      {/* 主要内容区域 */}
      <div
        className={`h-full bg-white rounded-xl border border-slate-200 shadow-sm transition-all duration-300 ease-in-out ${
      ...
      {isCollapsed && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-white rounded-xl border border-slate-200 shadow-sm">
          <div className="w-6 h-1 bg-slate-300 rounded-full mb-2 animate-pulse"></div>
          <div className="w-4 h-1 bg-slate-200 rounded-full mb-1"></div>
      ...
      <button
        ...
        className="absolute -right-4 top-1/2 transform -translate-y-1/2 z-20 
                   w-8 h-8 bg-white border-2 border-slate-300 rounded-full shadow-lg 
      ...
      >
        ...
        <div className="relative overflow-hidden w-5 h-5">
          {/* 展开图标 */}
          <ChevronRightIcon
            className={`absolute inset-0 w-5 h-5 text-slate-600 group-hover:text-blue-600 transition-all duration-300 transform ${
