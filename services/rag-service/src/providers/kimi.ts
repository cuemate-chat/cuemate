...
export class KimiEmbeddingProvider extends BaseEmbeddingProvider {
  private client: OpenAI | null = null;

  constructor(cfg: Record<string, any>) {
    super('kimi', {
      apiKey: cfg.api_key,
      baseUrl: cfg.base_url || 'https://kimi.moonshot.cn/api',
      // 传递其他动态字段
      ...Object.fromEntries(
  ...
  async embed(request: EmbeddingRequest): Promise<EmbeddingResponse> {
    ...
    try {
      ...
      logger.error({ err: error as any }, 'Kimi embedding failed');
      throw error;
    }
  }
}
