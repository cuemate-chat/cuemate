          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="bg-gray-50 dark:bg-gray-800 dark:bg-slate-900/30 p-4 rounded-lg text-sm text-gray-700 dark:text-slate-300 border border-gray-200 dark:border-gray-700 dark:border-slate-700 space-y-2">
                ...
                <div className="flex justify-between">
                  <span>块类型:</span>
                  <span className="font-medium text-blue-600 dark:text-blue-400">{getBlockTypeText(selectedBlock.type)}</span>
                </div>
                <div className="flex justify-between">
                  <span>广告价格:</span>
                  <span className="font-bold text-green-600 dark:text-green-400">¥{selectedBlock.price || 100}</span>
                </div>
              </div>
            </div>
              ...
              >
                {uploadedFile ? (
                  <div className="space-y-3">
                    ...
                    />
                    <div className="text-sm text-green-600 dark:text-green-400 font-medium">{uploadedFile.name}</div>
                    <div className="text-xs text-gray-500 dark:text-slate-400">点击更换图片</div>
                  </div>
                ) : (
                  <div>
                    <PhotoIcon className="w-12 h-12 text-gray-400 dark:text-slate-500 mx-auto mb-3" />
                    <div className="text-gray-600 dark:text-slate-400">
                      点击或拖拽图片到此处<br/>
                      <span className="text-xs text-gray-500 dark:text-slate-400 mt-1 block">支持 JPG, PNG, GIF 格式</span>
                    </div>
                  </div>
                )}
                <input
