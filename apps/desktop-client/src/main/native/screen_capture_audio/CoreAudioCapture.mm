...
#pragma mark - Core Audio Taps Implementation (macOS 14.2+)

// Core Audio Taps 处理器类
@interface CoreAudioTapsHandler : NSObject
@property (nonatomic, assign) BOOL isCapturing;
@property (nonatomic, copy) void (^audioDataCallback)(NSData *audioData);
@property (nonatomic, copy) void (^errorCallback)(NSError *error);
@end

@implementation CoreAudioTapsHandler
...
- (void)startCaptureWithSampleRate:(int)sampleRate channels:(int)channels {
    if (self.isCapturing) {
        coreAudioLogWarn("Core Audio Taps 捕获已在进行中");
        return;
    }

    coreAudioLogInfo("开始 Core Audio Taps 音频捕获...");

    // 简单的测试实现 - 生成测试音频数据
    self.isCapturing = YES;
    coreAudioLogInfo("Core Audio Taps 音频捕获已启动");

    // 在后台线程生成测试音频数据
    dispatch_queue_t audioQueue = dispatch_queue_create("CoreAudioTapsQueue", DISPATCH_QUEUE_SERIAL);
    dispatch_async(audioQueue, ^{
        while (self.isCapturing) {
            // 生成测试音频数据 (1024 samples, 16-bit)
            int16_t testAudio[1024];
            for (int i = 0; i < 1024; i++) {
                testAudio[i] = (int16_t)(sin(i * 0.01) * 1000); // 生成测试正弦波
            }

            NSData *audioData = [NSData dataWithBytes:testAudio length:sizeof(testAudio)];
            if (self.audioDataCallback) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    self.audioDataCallback(audioData);
                });
...
- (void)stopCapture {
    ...
    coreAudioLogInfo("停止 Core Audio Taps 音频捕获...");
    self.isCapturing = NO;
}
...
// C++ 包装器类实现
CoreAudioTapsWrapper::CoreAudioTapsWrapper() {
    CoreAudioTapsHandler *tapsHandler = [[CoreAudioTapsHandler alloc] init];
    tapHandler = (void*)tapsHandler;
}

CoreAudioTapsWrapper::~CoreAudioTapsWrapper() {
    if (tapHandler) {
        CoreAudioTapsHandler *tapsHandler = (CoreAudioTapsHandler*)tapHandler;
        [tapsHandler stopCapture];
        [tapsHandler release];
        tapHandler = nil;
    }
}

void CoreAudioTapsWrapper::startCapture(int sampleRate, int channels,
                                       std::function<void(const char*, size_t)> audioCallback,
                                       std::function<void(const std::string&)> errorCallback) {
    CoreAudioTapsHandler *tapsHandler = (CoreAudioTapsHandler*)tapHandler;

    // 设置音频数据回调
    tapsHandler.audioDataCallback = ^(NSData *audioData) {
        audioCallback((const char*)audioData.bytes, audioData.length);
    };

    // 设置错误回调
    tapsHandler.errorCallback = ^(NSError *error) {
        std::string errorMessage = error.localizedDescription.UTF8String;
        errorCallback(errorMessage);
    };

    [tapsHandler startCaptureWithSampleRate:sampleRate channels:channels];
}

void CoreAudioTapsWrapper::stopCapture() {
    if (tapHandler) {
        CoreAudioTapsHandler *tapsHandler = (CoreAudioTapsHandler*)tapHandler;
        [tapsHandler stopCapture];
    }
...
bool CoreAudioTapsWrapper::isCapturing() const {
    if (tapHandler) {
        CoreAudioTapsHandler *tapsHandler = (CoreAudioTapsHandler*)tapHandler;
        return tapsHandler.isCapturing;
    }
    return false;
}

bool CoreAudioTapsWrapper::isAvailable() {
    // 检查 macOS 版本是否支持 Core Audio Taps (需要 macOS 14.2+)
    if (@available(macOS 14.2, *)) {
        coreAudioLogInfo("系统版本支持 Core Audio Taps (macOS 14.2+)");

        // 进一步检查是否有音频权限
        // 在 macOS 上，Core Audio Taps 需要录音权限
        AVAuthorizationStatus microphoneStatus = [AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeAudio];

        switch (microphoneStatus) {
            case AVAuthorizationStatusAuthorized:
                coreAudioLogInfo("Core Audio Taps: 音频权限已授权");
                return true;

            case AVAuthorizationStatusNotDetermined:
                coreAudioLogWarn("Core Audio Taps: 音频权限未确定，需要请求权限");
                return false;

            case AVAuthorizationStatusDenied:
            case AVAuthorizationStatusRestricted:
                coreAudioLogError("Core Audio Taps: 音频权限被拒绝或受限");
                return false;

            default:
                coreAudioLogError("Core Audio Taps: 未知的音频权限状态");
                return false;
        }
    } else {
        coreAudioLogError("系统版本不支持 Core Audio Taps (需要 macOS 14.2+)");
        return false;
    }
}