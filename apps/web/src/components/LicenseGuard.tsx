...
export default function LicenseGuard({ 
  ...
  const { loading, isValid, isExpiringSoon, hasFeature } = useLicense();
  ...
  if (!isValid) {
    return fallback || (
      <div className="flex flex-col items-center justify-center min-h-[400px] p-8">
        ...
        <h3 className="text-lg font-medium text-slate-900 mb-2">需要有效授权</h3>
  ...
  if (isExpiringSoon) {
    return fallback || (
      <div className="flex flex-col items-center justify-center min-h-[400px] p-8">
        ...
        <h3 className="text-lg font-medium text-slate-900 mb-2">License 即将过期</h3>
        <p className="text-slate-600 text-center mb-4">
  ...
  if (feature && !hasFeature(feature)) {
    return fallback || (
      <div className="flex flex-col items-center justify-center min-h-[400px] p-8">
        <DocumentTextIcon className="h-16 w-16 text-slate-400 mb-4" />
        <h3 className="text-lg font-medium text-slate-900 mb-2">功能权限不足</h3>
        <p className="text-slate-600 text-center mb-4">
          您的 License 不包含此功能的访问权限
        </p>
        <a
          href="/settings/license"
          className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
