...
export class ZhipuEmbeddingProvider extends BaseEmbeddingProvider {
  private client: OpenAI | null = null;

  constructor(cfg: Record<string, any>) {
    super('zhipu', {
      apiKey: cfg.api_key,
      baseUrl: cfg.base_url || 'https://open.bigmodel.cn/api/paas/v4',
      // 传递其他动态字段
      ...Object.fromEntries(
  ...
  async embed(request: EmbeddingRequest): Promise<EmbeddingResponse> {
    ...
    try {
      ...
      logger.error({ err: error as any }, 'Zhipu embedding failed');
      throw error;
    }
  }
}
