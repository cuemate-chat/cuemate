/**
 * 面试训练AI分析服务
 * 专门处理面试训练结束后的AI分析、评分和报告生成
 */

import { InterviewScore, InterviewInsight } from '../../mock-interview/data/InterviewDataService';

// 面试训练分析请求接口
export interface TrainingAnalysisRequest {
  interviewId: string;
...
export class InterviewTrainingAnalysisService {
  ...
  async analyzeInterviewTraining(request: TrainingAnalysisRequest): Promise<TrainingAnalysisResult> {
    ...
    try {
      ...
      throw new Error(`面试训练分析失败: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   * 准备分析数据
   */
  private prepareAnalysisData(request: TrainingAnalysisRequest): any {
...
7. 确保返回标准JSON格式`;
   ...
   * 格式化分析结果
   */
  private formatAnalysisResult(aiAnalysis: any, _request: TrainingAnalysisRequest): TrainingAnalysisResult {
    // 确保所有字段都有默认值
    const result: TrainingAnalysisResult = {
      overallScore: Math.max(1, Math.min(100, aiAnalysis.overallScore || 75)),
      summary: aiAnalysis.summary || '面试训练分析完成',
  ...
  async saveAnalysisToDatabase(
    interviewId: string,
    analysisResult: TrainingAnalysisResult
  ...
  async saveQAAnalysisToDatabase(
    interviewId: string,
    qaAnalysis: TrainingAnalysisResult['qaAnalysis']
  ): Promise<string[]> {
    await this.ensureAuth();
  ...
  async analyzeAndSave(
    ...
    try {
      ...
      console.error('面试训练分析和保存流程失败:', error);
      throw error;
    }
  }
}

export const interviewTrainingAnalysisService = new InterviewTrainingAnalysisService();