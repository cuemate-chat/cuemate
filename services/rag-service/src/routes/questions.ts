...
export async function createQuestionRoutes(
  ...
  app.get('/questions/search', async (req) => {
    ...
    const { query, job_id, user_id, tag_id, top_k, question_title } = (req as any).query as {
      query: string;
      job_id?: string;
      user_id?: string;
      tag_id?: string;
  ...
  app.get('/questions/by-job/:jobId', async (req) => {
    ...
    // hook 已将 camelCase 转换为 snake_case
    const { top_k } = (req as any).query as { top_k?: string };
  ...
  app.post('/similarity/questions', async (req) => {
    ...
    try {
      const { query, job_id: jobId, threshold = 0.8, top_k: topK = 5 } = body;
  ...
  app.post('/similarity/questions/all', async (req) => {
    ...
    try {
      const { query, threshold = 0.8, top_k: topK = 3 } = body;
  ...
  app.post('/similarity/questions/interview', async (req) => {
    ...
    try {
      const { query, job_id: jobId, threshold = 0.8 } = body;
